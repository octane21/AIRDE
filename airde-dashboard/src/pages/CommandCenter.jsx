...
import { ShieldCheck, AlertTriangle, Hourglass, Gauge, Package, CheckCircle2, Target, Search, Wrench, BarChart3 } from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        <div className="p-4 md:p-5 space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            <KPICard icon={ShieldCheck} title="Asset Health Index (AHI)" value={kpi.ahi.value} label={kpi.ahi.label} trend="up" color="text-green-400" onClick={() => navigate('/risk')} />
            <KPICard icon={AlertTriangle} title="Risk Score (0-5)" value={kpi.riskScore.value} label={kpi.riskScore.label} trend="up" color="text-green-400" onClick={() => navigate('/risk')} />
            <KPICard icon={Hourglass} title="Remaining Life (AVG)" value={kpi.remainingLife.value} label={kpi.remainingLife.label} trend="up" color="text-blue-400" onClick={() => navigate('/risk?sort=remaining_life')} />
            <KPICard icon={Gauge} title="Inspection Coverage" value={kpi.inspectionCoverage.value} trend="up" color="text-yellow-400" onClick={() => navigate('/inspection?status=Pending')} />
            <KPICard icon={Package} title="Total Assets" value={kpi.totalAssets.value} label={kpi.totalAssets.label} sub={kpi.totalAssets.sub} color="text-slate-200" onClick={() => navigate('/asset')} />
          </div>
              ...
              <div className="space-y-2.5">
                {liveAlerts.map((alert, i) => (
                  <div
                    ...
                    className="flex gap-2.5 p-2.5 bg-[#111d35] rounded-md border border-[#1e2d4f] hover:border-orange-500/30 cursor-pointer transition-colors"
                  >
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-slate-400 whitespace-pre-line mt-0.5">{alert.message}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-[#0d1f3c] border border-[#1e2d4f] rounded-lg p-4">
              ...
              <div className="space-y-2.5">
                {liveKeyTakeaways.map((kt, i) => (
                  <div key={i} className="flex gap-2.5 text-[11px] text-slate-400">
            ...
            <div className="bg-[#0d1f3c] border border-[#1e2d4f] rounded-lg p-4">
              ...
              <div className="space-y-2.5">
                {liveNextActions.map((action, i) => {
                  const Icon = NEXT_ACTION_ICONS[action.icon] || Target;
                  return (
                    <div key={i} className="flex gap-2.5 p-2.5 bg-[#111d35] rounded-md border border-[#1e2d4f] hover:border-orange-500/30 transition-colors cursor-pointer">
                      <Icon size={15} className="text-orange-400 flex-shrink-0 mt-0.5" strokeWidth={1.75} />
